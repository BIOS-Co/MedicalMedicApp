...
import {
  formatearHora,
  getAge,
} from "../../../services/DateManagement/DateManagement";
import CustomModal from "../../../Shared/Alerts/Alert";
import LoadingScreen from "../../../Shared/Alerts/Loader";
import {
  UpdateDate_Arrive,
  UpdateDate_FINISH,
  Whatsapp_message_llegada,
  Whatsapp_message_time,
  getActivities,
} from "../../../services/MainApp/NewService/NewServiceForm/NewServiceForm";
import AlertComponent from "../../../Shared/Icons/AlertComponent";
import {
  getActiveDates,
  getNotifications,
  getNotificationsMaintenance,
} from "../../../services/MainApp/HistoryDates/HistoryDates";
...
export default function Lobby(props) {
  ...
  /* APP CONTEXT */
  var {
    Notification_basic_medic,
    setNotification_basic_medic, // variable para cargar las notificaciones del inventario del médico
    Notification_Maintenance_medic,
    setNotification_Maintenance_medic, // variable para cargar las notificaciones de mantenimiento del médico
    activities,
    setActivities, // variable para cargar los servicios que maneja MHC
    userData,
    setUserData, // variable para manejar la información del usuario
    token,
    setToken, // token de acceso a los servicios
    currentDate,
    setCurrentDate, // variable para almacenar la cita que este activa en el momento o en su defecto la ultima cita cargada
    currentPosition,
    setCurrentPosition, // variable para almacenar la posición actual del médico
    dateAcceptedMessage,
    setDateAcceptedMessage, // variable para saber si le manda el mensaje a whatsapp de que ya está en camino
  ...
  const handleMinutesError = () => {
    setMessage('Aún no han pasado 20 minutos desde que inició la consulta');
    setIconName('error');
    setShowModal(true);
  };
  ...
  const GetNotifications = async () => {
    ...
    if (result !== undefined) {
      if (Object.keys(result.data).length === 0) {
        ...
        */
        setNotification_basic_medic([
          { info: "Debes acercate al almacén para recargar inventario básico" },
        ]);
      }
    }
  };
  ...
  const GetActividad = (ActivityId) => {
    ...
    let FilterArray = activities.filter(
      (obj) => obj.value.toString() === ActivityId.toString()
    );
  ...
  */
    ...
    let date2 = moment(currentDate.datetime_arrival);
    let minutosTranscurridos = date1.diff(date2, 'minutes');
  ...
  */
  ...
  return (
    <View style={styles.container}>
      ...
      <ImageBackground
        source={require("../../../assets/Home/BG-Particular.png")}
      ...
      >
        <View style={styles.LobbyContainer}>
          ...
          <LinearGradient
            ...
            style={[styles.FormContainer, { minHeight: windowHeight - 200 }]}
          >
            {currentDate !== null ? (
              <>
                <Text
                ...
                >
                  ...
                  >
                    {currentDate?.genre?.toLowerCase() === "masculino" ? (
                      ...
                      >
                        <ImageBackground
                          source={require("../../../assets/Female-User.png")}
                          style={styles.photo}
                        ></ImageBackground>
                      </View>
                    )}
                    ...
                    >
                      <View style={{ width: "100%" }}>
                        ...
                        >
                          {currentDate.identification_type +
                            " " +
                            currentDate.identification}
                        </Text>
                      </View>
                    </View>
                  </View>
                  ...
                  >
                    ...
                    >
                      {GetActividad(currentDate?.activity)}
                    </Text>
                  </View>
                  <View
                    style={{
                  ...
                  {destinationCoordinates !== null ? (
                    <>
                      {currentDate?.datetime_arrival !== null && currentDate?.activity === 2 ? (
                        <></>
                      ) : (
                        <View
                          style={{
                    ...
                    >
                      ...
                      >
                        {"No fue posible encontrar la dirección en google maps"}
                      </Text>
                      <ConsultaDomestica
                  ...
                  {currentDate?.datetime_arrival !== null ? (
                    <TouchableOpacity
                      style={{ ...styles.buttonDelete, borderColor: "#3A9EE9" }}
                      onPress={UPDATE_DATE_FINISH}
                    >
                      <Text
                        style={{
                          ...styles.buttonText,
            ...
            >
              <View style={{ width: "100%", flexDirection: "row" }}>
                <View style={{ maxHeight: 100, flexDirection: "row" }}>
                  <TouchableOpacity
                    style={{ ...styles.options, ...styles_shadow, width: 250 }}
                  ...
                  >
                    ...
                    >
                      <Text style={{ color: "white" }}>
                        {Notification_Maintenance_medic.length +
                          Notification_basic_medic.length}
                      </Text>
                    </View>
                    <View
                      style={{
